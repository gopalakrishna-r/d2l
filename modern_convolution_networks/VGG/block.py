...
from tensorpack.models import  FullyConnected,  Dropout, MaxPooling, AvgPooling, Conv2D, BatchNorm, Flatten

def vgg_block(name, num_convs, num_channels, input):
    for i in range(num_convs):
        input = Conv2D(f'{name}_{i}_conv', input, num_channels, kernel_size=3, padding='SAME', activation=tf.nn.relu)
    input = MaxPooling(f'{name}_max_pooling',input, pool_size=2, strides=2)
    return input
