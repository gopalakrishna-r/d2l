...
def extract_joke(id):
    ...
    category_rating_cells = content.xpath(
        '//table[@bgcolor="#ffffff"]//table[@class="bkline"]//td/b[text()="Category: "]/..')
    # print(category_rating_cell)

    crap = content.xpath(
        './child::node()[not(self::text()) and not(self::br)]')  # all html nodes in content, but not plaintext
