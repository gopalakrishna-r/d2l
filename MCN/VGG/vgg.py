...
def vgg_plain(convo_arch):
    bridged_input = tf.keras.Input(shape=(96, 96, 1))
    l = bridged_input
    with slim.arg_scope([dense], units=4096), \
            slim.arg_scope([dense], activation=tf.nn.relu), \
        ...
        return tf.keras.Model(bridged_input, logits)


class VGGStem(tf.keras.Model):
...
def vgg_block(inputs, name, num_convs, num_channels, reuse=None, scope=None):
    with variable_scope.variable_scope(
        ...
        layer = VGGStem(name=name, num_convs=num_convs, num_channels=num_channels)
