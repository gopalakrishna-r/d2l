...
class B1(tf.keras.Model):
    def __init__(self, name):
        super().__init__()
        self.conv = conv2d(name = f'{name}_conv', filters = 64, kernel_size = 7, strides = 2, padding='SAME', activation=tf.nn.relu)
...
class B2(tf.keras.Model):
    def __init__(self, name):
        ...
        self.conv = conv2d(name = f'{name}_conv', filters = 192, kernel_size = 3, padding='SAME', activation=tf.nn.relu)
        self.max_pool = max_pool2d(name = f'{name}_max_pool', pool_size = 3, strides = 2, padding='SAME')
...
def build_configs(block):
    return (tuple(zip(block['path_1_configs'], block['path_2_configs'], block['path_3_configs'], block['path_4_configs'])))



def build_graph():
    inception_block_cofigs = {
        'b3': {'path_1_configs':[64, 128], 'path_2_configs': [(96, 128), (128, 192)], 'path_3_configs': [(16, 32), (32, 96)], 'path_4_configs': [32, 64]},
        'b4': {'path_1_configs':[192, 160, 128, 112, 256], 'path_2_configs': [(96, 208), (112, 224), (128, 256),(144, 288),  (160, 320)], 
               'path_3_configs': [(16, 48), (24, 64),(24, 64),(32, 64), (32, 128)], 'path_4_configs': [64, 64,64,64, 128]},
        'b5': {'path_1_configs':[256, 384], 'path_2_configs': [(160, 320), (192, 384)], 'path_3_configs': [(32, 128), (48, 128)], 'path_4_configs': [128, 128]},
    }
