...
def build_graph(hp : HyperParameters):
        ...
        nin_stem_configs = tuple(zip(channel_configs, kernel_size_configs,strides_configs, padding_configs, max_pool_configs))
        
        with slim.arg_scope([dropout], rate = 0.5):
                l =   slim.stack( l, nin_block , [(f'nin_block_{i}',tuple[0], tuple[1], tuple[2],tuple[3], tuple[4]) for i, tuple in enumerate(nin_stem_configs[:-1])])
                l =   dropout(name ='dropout')(l)
                l =   slim.stack( l, nin_block , [(f'nin_block_{len(nin_stem_configs)-1}',nin_stem_configs[-1][0], nin_stem_configs[-1][1], nin_stem_configs[-1][2],nin_stem_configs[-1][3],nin_stem_configs[-1][4])])
...
def nin_block(inputs , name, num_channels, kernel_size, strides, padding, enable_max_pool = True, reuse=None, scope=None):
  with variable_scope.variable_scope(
    ...
    layer = nin_stem(name, num_channels, kernel_size, strides, padding, enable_max_pool)
...
class nin_stem(tf.keras.Model):
  def __init__(self, name, num_channels, kernel_size , strides, padding, enable_max_pool):
      super().__init__(name = name)
      self.conv_1 = conv2d(name = f'{name}_conv1',  filters = num_channels, kernel_size = kernel_size, strides = strides, padding = padding, activation = 'relu')
      self.conv_2 = conv2d(name = f'{name}_conv2',  filters = num_channels, kernel_size = 1, strides=1, padding='SAME', activation = 'relu')
      self.conv_3 = conv2d(name = f'{name}_conv3',  filters = num_channels, kernel_size = 1, strides=1, padding='SAME', activation = 'relu')
