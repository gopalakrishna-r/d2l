...
def build_rnn_model(config):
    gru_cell = tf.keras.layers.GRUCell(config.hidden_units, kernel_initializer='glorot_uniform')
    gru_layer = tf.keras.layers.RNN(gru_cell, time_major=True, return_state=True, return_sequences=True)
    return d2l.RNNModel(gru_layer, vocab_size=vocab_size)


gpu_device_name = d2l.try_gpu()._device_name
with wandb.init(project="hyperparameter-sweeps-rnn", name='gru-rnn', entity='goofygc316') as run:
